...
export class ConfigHelper extends EventEmitter {
  ...
  private sanitizeModelSelection(model: string, provider: "openai" | "gemini" | "anthropic"): string {
    if (provider === "openai") {
      ...
      const allowedModels = ['gpt-4o', 'gpt-4o-mini', 'meta-llama/llama-4-scout-17b-16e-instruct', 'openai/gpt-oss-120b'];
  ...
  public updateConfig(updates: Partial<Config>): Config {
    try {
      ...
      if (updates.apiKey && !updates.apiProvider) {
        ...
        if (updates.apiKey.trim().startsWith('sk-') || updates.apiKey.trim().startsWith('sk-or-') || updates.apiKey.trim().startsWith('gsk_')) {
          if (updates.apiKey.trim().startsWith('sk-ant-')) {
            provider = "anthropic";
            console.log("Auto-detected Anthropic API key format");
          } else {
            provider = "openai";
            console.log("Auto-detected OpenAI/OpenRouter API key format");
          }
        } else {
          provider = "gemini";
  ...
  public isValidApiKeyFormat(apiKey: string, provider?: "openai" | "gemini" | "anthropic" ): boolean {
    ...
    if (!provider) {
      if (apiKey.trim().startsWith('sk-') || apiKey.trim().startsWith('sk-or-') || apiKey.trim().startsWith('gsk_')) {
    ...
    if (provider === "openai") {
      ...
      return /^(sk-[a-zA-Z0-9\-_]{20,}|gsk_[a-zA-Z0-9\-_]{20,})$/.test(apiKey.trim());
    } else if (provider === "gemini") {
    ...
    if (!provider) {
      if (apiKey.trim().startsWith('sk-') || apiKey.trim().startsWith('sk-or-') || apiKey.trim().startsWith('gsk_')) {
        if (apiKey.trim().startsWith('sk-ant-')) {
          ...
          provider = "openai";
          console.log("Auto-detected OpenAI/OpenRouter API key format for testing");
        }
      } else {
        provider = "gemini";
    ...
    try {
      ...
      if (apiKey.startsWith('sk-or-')) {
        ...
        clientOptions.defaultHeaders = {
           "HTTP-Referer": "https://github.com/j4wg/interview-coder-withoupaywall-opensource",
           "X-Title": "CodeInterviewAssist"
        };
